...
def LinewidthpstopFunc(folderIn, folderOut, fileXLSX, fileIn):
    ...
    n8 = fileIn.split('_')[8]
    if (n8 == 'pstopLinewidth'):
            struct = 'LINEWIDTH_STOP'
            waitTime = '0.200'
    ...
    procedureType = ET.SubElement(data, "PROCEDURE_TYPE").text = 'Linewidth-pstop_2'
    ...
    sheet = wb_obj.active
    t = 128.5*(sheet["B5"].value)/(sheet["B12"].value)
    t = round(t, 3)
